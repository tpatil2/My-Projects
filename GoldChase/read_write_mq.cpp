...
void read_Dmessage(int pnum)
{
  ...
  mq_notify(Dreadqueue_fd[pnum], &mq_notification_event);

  //read a message
  int err;
  char msg[121];
  memset(msg, 0, 121);//set all characters to '\0'
  while((err=mq_receive(Dreadqueue_fd[pnum], msg, 120, NULL))!=-1)
  {
...
const char *str = st.c_str();
  ...
  if(mq_name1=="/none")
  {
    cerr << "Please change mq_name1 from \"" << mq_name1 << "\" to something unique to you" << endl;
    exit(1);
  }

  mqd_t writequeue_fd; //message queue file descriptor

  //cout<<"mqq in write is--"<<mq_name1<<endl;
  if((writequeue_fd=mq_open(mq_name1.c_str(), O_WRONLY|O_NONBLOCK))==-1)
  {
    perror("mq_open");
    exit(1);
  }
...
//  action_to_take.sa_handler=read_Dmessage;
  ...
  if((Dreadqueue_fd[pnum]=mq_open(mq_name1.c_str(), O_RDONLY|O_CREAT|O_EXCL|O_NONBLOCK,
